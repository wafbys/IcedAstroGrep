...
- **M0 spike 脚手架（现为 `spikes/winui-shell/`，原名 `winui-m0`）**：最小 WinUI 3 窗口（Windows App SDK 2.4.0、免打包、x64），只引用 `IcedAstroGrep.Core` 与 `IcedAstroGrep.AppServices`，窗口里显示构建标识（Core）、内置插件数量（AppServices，顺带走到内嵌的 `pdftotext` 与设置文件）与一条本地化文案（语言下沉的成果）。**刻意不加入 `IcedAstroGrep.slnx`**，因此 `dotnet build`、`dotnet test` 与 CI 都不会去还原 Windows App SDK，主构建保持 0 警告。README 写明验收标准、运行命令、以及自包含与框架依赖两种配置的差别。
- **M0 已在真机验证通过（2026-09-12）**：该 spike 在具备可用 NuGet 连接的机器上**构建并运行成功**，因此三件事得到确认：工具链可用（`net10.0-windows10.0.19041.0` + WASDK 2.4.0 + 免打包 + x64）；**WinUI 3 工程可以引用 `net10.0-windows` 的 Core 与 AppServices**（平台版本是下限，可引用更低版本）；引擎侧三层（Core 的构建标识、AppServices 的插件与语言文案）都能在 WinUI 进程里工作。这关掉了评估文档里最后一条"未核实"。
- **M1（WinUI 搜索闭环）已写出，待在真机验证**：spike 上移为 `spikes/winui-shell/` 并实现搜索闭环——路径 / 搜索词 / 文件类型 + 子目录、大小写、全词、正则、否定、仅文件名；开始与取消；进度环与实时状态行（搜索文件数、命中文件数、命中行数、错误数）；随命中增长的文件列表；结果面板**由共享的 `Display.ResultDocument` 组装**（布局与 WinForms 面板一致，未重写一行）。十个事件全部订阅并编组到 `DispatcherQueue`，**逐行事件刻意不编组**（每命中一行触发一次，会把 UI 线程淹掉）只计数、由文件级事件刷新状态。取消顺序按契约写死在 `StartSearchClick`：**先解绑十个处理器 → `AbortAndWait` → 再开新搜索**，注释写明为什么不能省（两个搜索会争用插件实例与磁盘编码缓存）。失败**被显示而不是被计数**：`SearchRegexTimeoutException` 打印 `SEARCH STOPPED: …`。仍未做：查看器高亮与点击定位（M2，等查看器路线决定）、设置/插件/编辑器界面与 `IUserNotifier`（M3）。本机无法构建 WASDK 工程（大传输被截断），按 M0 已验证的方式由用户在真机构建运行后回馈。
- **M1 已在真机构建并运行成功（2026-09-12）**：搜索闭环在新壳里跑通（文件列表与结果面板填充、Cancel 生效）。
- **M2（WinUI 结果查看器，路线 A：WebView2）已写出，待在真机验证**：结果面板改为 `WebView2`，显示的就是**导出用的那份 HTML**——导出器新增 `MatchResultsExport.BuildResultsAsHTML(settings)`（`SaveResultsAsHTML` 现在调用它写文件），因此面板、导出文件与打印页是同一份文档，不可能漂移；测试 `TheBuiltDocumentIsExactlyWhatTheFileExportWrites` 断言"文件内容 == 字符串内容"。重构后实测导出 HTML 与重构前**逐字节相同**（4315 字节含 BOM，用 CLI 导出比对的基准法）。**点击定位**：导出器新增 `IncludeSourceLocations`（默认关闭，故导出文件保持逐字节不变），打开时每行带 `data-file`/`data-line`/`data-column`，注入的小脚本把点击回传 `WebMessageReceived` → `TextEditors.Open(opener, notifier)`（顺带用上 M3a 的通知接缝）。**打印**走 `CoreWebView2.ShowPrintUI()`——这正是路线 A 的红利，WinUI 3 自身没有打印能力。新增 `HtmlExportTests`（4 个用例：可只构建不落盘、文件与字符串完全相等、源位置默认不出现、开启后属性与高亮都正确）。注：§3.1 的 `ResultDocument` 不是本面板的渲染来源（用的是导出标记），它仍是 WinForms 面板与路线 B/C 的定义。
- **M3a（WinUI 设置 / 语言 / 通知）已写出，待在真机验证**：语言选择框由 `Language.AvailableLanguages` 填充，切换时 `Language.Load` + 存 `GeneralSettings.Language` + 保存设置；此后引擎的状态文案（`SearchStarted`/`SearchSearching`/`SearchFinished`/`SearchCancelled`/`SearchFileError`）是**本地化**的，而壳自己的标签保持自己的措辞——正是契约里写的分工。搜索选项（子目录/大小写/全词/正则/否定/仅文件名）与上下文行数从**共享设置文件**读取、搜索开始时写回 `SearchSettings` 并保存，因此两个壳能互相看到对方的设置（切换壳不丢配置）。新增 `WinUiNotifier` 实现 `IUserNotifier`（`ContentDialog`，`XamlRoot` 首次使用时才取——窗口的 XamlRoot 在 `InitializeComponent` 时还不存在），并用于唯一值得弹窗的消息：正则匹配超时导致搜索中止。仍未做（M3b）：完整的选项页、排除项编辑、文本编辑器配置、插件管理。
- **WinUI 壳界面按微软 Fluent/WinUI 指南重做（M4 的一部分，待在真机验证）**：布局落到 4px 间距网格与 24px 页边距；**XAML 里不再有任何 `CornerRadius`**（圆角交给框架的 `ControlCornerRadius`/`OverlayCornerRadius`）、**不再有任何硬编码颜色与 `FontSize`**（深色模式因此自动成立，也让"猜主题资源键"这种会直接崩掉 XAML 的风险归零），层次改由类型阶梯 `CaptionTextBlockStyle` 表达；输入控件的标签改用各自的 `Header`（同时就是无障碍标签）；报错从红字文本块改为 `InfoBar`（`Severity="Error"`），只有"搜索被中止"仍用对话框；键盘交互补齐 **Enter 搜索、Esc 取消**（挂在 Cancel 按钮上，而该按钮在搜索未运行时禁用，因此 Esc 不会误触）、**Ctrl+F 聚焦搜索框**、**Alt+S / Alt+C 访问键**；搜索期间禁用输入面板，避免中途改动导致结果与条件不符；窗口标题显示构建标识、初始尺寸 1100×760、加载后焦点落在搜索框；文件列表与结果面板补 `AutomationProperties.Name`。规则与逐条对照表写进了 `spikes/winui-shell/README.md`，待回馈的验收项也同步加了键盘与深色模式两项。**首次真机构建暴露了四处 WinUI 与 WPF 的差异并已修**：`ProductInformation` 在 `IcedAstroGrep.Core` 里（只 `using IcedAstroGrep;` 会 CS0103）；WinUI 的 `UIElement` **没有** `IsEnabled`，不能用 WPF 那种"禁用整个容器"的写法，改为逐个 `Control` 切换（顺带让键盘与读屏软件也能感知禁用状态，而不是只做视觉变暗）；当前 WebView2 SDK 的 `ShowPrintUI` 要求 `CoreWebView2PrintDialogKind`；以及 `WMC9999`/`WMC1509` 是 C# 编译失败后的**连锁**（标记编译第二趟拿不到本项目的程序集），C# 错误修好后即消失——这条差异表也写进了 README，供下一个写壳的人少走一趟。仍未做（有意）：壳自身标签仍是英文字面量，按指南应改为 `.resw` + `x:Uid`，留到 M3b 与页面文案一起定，避免现在做一次无法验证的改动。
- **结果展示模型下沉到 AppServices（§3.1）**：新增 `IcedAstroGrep.AppServices/Display/ResultDocument.cs`——`ResultDocument`（面板文本 + 每行的源位置映射）、`ResultDocumentLine`（不可变：文本、种类、源文件、源行号、列号、是否命中）、`ResultDocumentOptions`（是否去前导空白、上下文行数）。`frmMain.ProcessAllMatchesForDisplay` 里约 55 行的组装代码改为"构建模型 → 投影成壳的 `LineNumber` 列表并赋给控件"，文本与映射逐字不变。仍留在壳侧的是**有意为之**的渲染适配：单文件预览会把整个文件 `Load` 进 AvalonEdit（语法高亮属于控件能力）、高亮器按 `MatchResult` 自行重算着色、点击定位依赖 AvalonEdit 的锚点。
- **验证方式（本项目一贯做法：先记基准，再逐字比对）**：改动前给面板加临时 dump（面板文本 + 行号映射，转义为单行写进日志），用固定 fixture（2 个文件、含缩进行、命中在第 2 行）跑一次真实程序并把日志行存为基准；改动后再 dump 一次，剔除日志时间戳前缀后 `-ceq` 比对 → **864 字符完全相同**，确认重构未改变任何呈现。临时 dump 已在同一提交前移除。
- **结果面板首次获得测试覆盖**：新增 `ResultDocumentTests`（6 个用例）——文档以"路径 + 空行 + 命中行"开头、源位置与 1-based 列号正确、文本以平台换行拼接且**末尾无换行**、上下文行被标为 context、两个文件之间是**两个空行**、去前导空白时"命中行保留命中之前的缩进而上下文行整体 TrimStart"、无结果时文档为空。写测试时顺带确认了一个架构细节：**上下文行是搜索时捕获的**（`SearchSpec.ContextLines`），显示选项只是从已捕获的行里挑选——新壳调整上下文行数时必须让搜索也带上上下文。
