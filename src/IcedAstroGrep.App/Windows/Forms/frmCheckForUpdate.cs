...
namespace IcedAstroGrep.Windows.Forms
{
   ...
   public partial class frmCheckForUpdate : BaseForm
   {
      ...
      private void CheckForUpdate()
      {
         try
         {
            ...
            using (WebResponse webRes = webReq.GetResponse())
            {
               ...
               // current version
               Version currentVersion = IcedAstroGrep.Core.ProductInformation.ApplicationVersion;

               //if (currentVersion.CompareTo(newestVersion) < 0)
               //{
               GetLatestChangeLog();
               UpdateMessage(string.Format("A newer version of IcedAstroGrep is available: {0}", newestVersion.ToString(3)), newestVersion.ToString(3));
               //}
               //else
               //{
      ...
      {
         // Download url example:
         // http://downloads.sourceforge.net/astrogrep/IcedAstroGrep_v4.3.1.zip
         ...
         try
         {
            if (btnNext.Text == "Finish")
            {
               ...
               if (File.Exists(installFile) && (new FileInfo(installFile).Length > 0))
               {
                  ...
                  // close all IcedAstroGrep instances but our own
            ...
            else
            {
               ...
               string file = string.Format("IcedAstroGrep_Setup_v{0}.exe", newestVersion);
