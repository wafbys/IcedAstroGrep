		...
		private void CheckForUpdate()
		{
			try
			{
				...
				using (WebResponse webRes = webReq.GetResponse())
				{
					...
					IcedAstroGrep.Core.Logging.LogClient.Instance.Logger.Info($"Successfully checked for new version, retrieved version {newestVersion.ToString(3)}, current version: {currentVersion.ToString(3)}");
				}
			}
			catch (Exception ex)
			{
				IcedAstroGrep.Core.Logging.LogClient.Instance.Logger.Warn(ex, $"Unable to check for update: {ex.Message}");
