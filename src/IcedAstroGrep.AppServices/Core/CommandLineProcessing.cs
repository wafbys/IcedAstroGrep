...
namespace IcedAstroGrep
{
	...
	public class CommandLineProcessing
	{
		...
		public class ValueOptionPair
		{
			...
			/// <summary></summary>
			public IcedAstroGrep.Core.FilterType.ValueOptions ValueOption { get; set; }
		}
		...
		private static IcedAstroGrep.Core.FilterType.ValueOptions GetValueOptionFromOperator(string op)
		{
			IcedAstroGrep.Core.FilterType.ValueOptions valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.None;

			switch (op)
			{
				case "=":
					valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.Equals;
					break;

				case "!=":
					valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.NotEquals;
					break;

				case ">":
					valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.GreaterThan;
					break;

				case ">=":
					valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.GreaterThanEquals;
					break;

				case "<":
					valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.LessThan;
					break;

				case "<=":
					valueOption = IcedAstroGrep.Core.FilterType.ValueOptions.LessThanEquals;
					break;
			}
		...
		private static void ProcessFlags(Arguments myArgs, ref CommandLineArguments args)
		{
			...
			if (myArgs["dmf"] != null)
			{
				...
				if (values.Length == 2)
				{
					IcedAstroGrep.Core.FilterType.ValueOptions valueOption = GetValueOptionFromOperator(values[0]);
					if (DateTime.TryParse(values[1], out DateTime value) && valueOption != IcedAstroGrep.Core.FilterType.ValueOptions.None)
					{
			...
			if (myArgs["minfsize"] != null)
			{
				...
				{
					IcedAstroGrep.Core.FilterType.ValueOptions valueOption = GetValueOptionFromOperator(values[0]);
					if (long.TryParse(values[1], out long value) && valueOption != IcedAstroGrep.Core.FilterType.ValueOptions.None)
					{
