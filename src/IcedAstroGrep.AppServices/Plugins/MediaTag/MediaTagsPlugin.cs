...
namespace IcedAstroGrep.Plugins.MediaTag
{
	/// <summary>
	/// Used to search media files for a specified string using TagLib to read common properties.
	/// </summary>
	/// <remarks>
	/// IcedAstroGrep File Searching Utility. Written by Theodore L. Ward Copyright (C) 2002 AstroComma Incorporated.
	///
	...
	public class MediaTagsPlugin : IDisposable, IIcedAstroGrepPlugin
	{
		...
		private void GrepTag(string tagTitle, string tagValue, FileInfo file, ISearchSpec searchSpec, Regex reg, ref MatchResult match)
		{
			...
			else
			{
				...
				if (searchSpec.UseWholeWordMatching)
				{
					...
					if (mtc != null && mtc.Success && IcedAstroGrep.Core.Grep.RetrieveLineMatches(tagValue, searchSpec).Count > 0)
					{
						posInStr = mtc.Index;
					}
				}
			...
			if (posInStr > -1)
			{
				...
				{
					var lineMatches = IcedAstroGrep.Core.Grep.RetrieveLineMatches(tagValue, searchSpec);
